...
async def _generate_avatar_response(
    ...
    """
    try:
        # 构建系统提示 - 明确要求基于topic表达态度
        system_prompt = f"""你是数字分身{avatar_name}，一个AI驱动的虚拟人格。你正在参与一个关于"{scenario['name']}"的社会模拟。

场景氛围：{scenario['atmosphere']}
你的角色：{avatar_role}

**重要**：你需要针对下面提到的事件/话题，表达你真实的立场和态度：
- 如果你支持这个话题，sentiment 应该是 0.3 到 1.0 之间的正值
- 如果你反对这个话题，sentiment 应该是 -0.3 到 -1.0 之间的负值
- 如果你持中立态度，sentiment 应该在 -0.3 到 0.3 之间

你需要生成：
1. 内心思考过程（thinking）- 用第一人称描述你对这个话题的看法、为什么支持或反对
2. 实际发言（message）- 你会对外说什么，明确表达你对这个话题的立场
3. 情感态度（sentiment）- 基于你对话题的态度，给出 -1 到 1 之间的数值
4. 表情符号（emotion_emoji）- 符合你态度的表情
